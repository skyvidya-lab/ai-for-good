...
add_markdown("# Zero Hunger - Top-1 Strategy V3: L-TAE (Crop + Region + Hurst) & L-TAE (Rice Pheno)\n\nVersão Final para o Ouro. Adicionado Contexto Espacial (Região) e Memória Longa (Hurst) para o modelo de Crop.")
...
add_code("""# ─── Cell 3 — L-TAE Features: Soft Labels, DOY, Region, Hurst
import torch
...
# Region and Hurst Mapping
unique_regions = sorted(set(ps.region for ps in series_agro))
region_to_idx = {r: i + 1 for i, r in enumerate(unique_regions)}
...
for i, ps in enumerate(series_agro):
    ...
    region_ids[i] = region_to_idx[ps.region]
...
add_code("""# ─── Cell 4 — L-TAE Core Architecture
import math
...
class LTAECrop(nn.Module):
    def __init__(self, input_dim, n_crops=3, d_model=128, n_regions=10, region_emb_dim=16):
        super().__init__()
        self.region_emb = nn.Embedding(n_regions + 1, region_emb_dim, padding_idx=0)
        # Input to core: Satellite(input_dim) + Region(region_emb_dim) + Hurst(1)
        self.core = LTAECore(input_dim + region_emb_dim + 1, d_model=d_model, n_layers=2)
        self.crop_head = nn.Linear(d_model, n_crops)
        
    def forward(self, x, doy, region_id, hurst, pad_mask=None):
        B, T, _ = x.shape
        r_emb = self.region_emb(region_id).unsqueeze(1).expand(-1, T, -1)
        h_exp = hurst.unsqueeze(1).unsqueeze(2).expand(-1, T, 1)
        
        x_in = torch.cat([x, r_emb, h_exp], dim=-1)
        h = self.core(x_in, doy, pad_mask)
        
        # Average pooling over valid timesteps
        valid_mask = ~pad_mask
...
class LTAERicePhenology(nn.Module):
    def __init__(self, input_dim, n_pheno, d_model=128):
        super().__init__()
        self.core = LTAECore(input_dim, d_model=d_model, n_layers=3)
...
add_code("""# ─── Cell 5 — Train L-TAE for CROP TYPE (With Region & Hurst)
...
for fold, (tr, va) in enumerate(FOLDS):
    ...
    model = LTAECrop(F_AGRO, n_crops=3, n_regions=N_REGIONS).to(DEVICE)
    ...
    ds = TensorDataset(
        ...
        torch.tensor(region_ids[tr], dtype=torch.long),
    ...
    for epoch in range(40):
        model.train()
        for xb, doyb, rb, hb, padb, yb in dl:
            xb, doyb, rb, hb, padb, yb = (t.to(DEVICE) for t in (xb, doyb, rb, hb, padb, yb))
            opt.zero_grad()
            logits = model(xb, doyb, rb, hb, padb)
            loss = crit(logits, yb)
            loss.backward()
            opt.step()
        ...
        with torch.no_grad():
            ...
            r_v = torch.tensor(region_ids[va], dtype=torch.long).to(DEVICE)
            ...
            out_v = model(x_v, doy_v, r_v, h_v, pad_v)
...
add_code("""# ─── Cell 6 — Train L-TAE for RICE PHENOLOGY (Untouched, it is already #1)
...
regions_rice = region_ids[rice_indices]
...
f1_pheno_global = f1_score(pheno_y_strict[strict_rice_mask], pheno_preds_global[strict_rice_mask], average='macro', zero_division=0)

final_score = 100.0 * (0.5 * f1_crop_global + 0.5 * f1_pheno_global)

print(f"\\n====== TOP-3 STRATEGY V3 (FINAL) RESULTS ======")
print(f"F1 Crop (L-TAE w/ Region+Hurst): {f1_crop_global:.4f}")
print(f"F1 RicePheno (L-TAE):            {f1_pheno_global:.4f}")
print(f"FINAL LEADERBOARD SCORE:         {final_score:.2f} / 100")
if final_score > 98.22:
    print("🥇 EXPECTED 1ST PLACE OVERALL! 🥇")
elif final_score > 97.96:
