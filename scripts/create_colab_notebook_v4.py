...
add_markdown("# Zero Hunger - Top-1 Strategy V4: The Golden Fix\n\nCorreção do erro de Region Embedding e otimização de estabilidade.")
...
class LTAECrop(nn.Module):
    def __init__(self, input_dim, n_crops=3, d_model=128):
        super().__init__()
        # Input: Satellite + Hurst Exponent (1 dimension)
...
for fold, (tr, va) in enumerate(FOLDS):
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, doyb, hb, padb, yb in dl:
            ...
            # Time-Step Dropout for Crop (crucial for robustness against unseen regions)
            aug_padb = padb.clone()
...
print(f"\\n====== TOP-3 STRATEGY V4 (FINAL) RESULTS ======")
print(f"F1 Crop (L-TAE w/ Hurst): {f1_crop_global:.4f}")
