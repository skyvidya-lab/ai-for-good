...
add_markdown("# Zero Hunger - Top-3 Strategy V2: L-TAE (Crop) + L-TAE (Rice Pheno)\n\nLGBM substituído por L-TAE para Crop Type.")
...
class LTAECrop(nn.Module):
    def __init__(self, input_dim, n_crops=3, d_model=128):
        super().__init__()
        self.core = LTAECore(input_dim, d_model=d_model, n_layers=2)
...
add_code("""# ─── Cell 5 — Train L-TAE for CROP TYPE (Fixing LGBM F1 Drop)
...
for fold, (tr, va) in enumerate(FOLDS):
    ...
    # Class weights for focal/CE loss
...
add_code("""# ─── Cell 6 — Train L-TAE for RICE PHENOLOGY
...
print(f"Training L-TAE Rice Pheno on {DEVICE}...")
